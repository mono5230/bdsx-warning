            ...
            if (actor?.isPlayer()) {
                ...
                if (params.WarningScore < 0) {
                    ...
                    if (read1.warning < 0) {
                        ...
                        const read2 = JSON.parse(fs.readFileSync(UserJSON, "utf8"))

                        bedrockServer.executeCommand(`tellraw "${params.target.getName()}" {"rawtext":[{"text":"§l§d[ §fServer §d] §r: 당신의 경고가 ${params.WarningScore}차감 되었습니다! §c현재 경고 수: §f( ${read2.warning}/${warningban} )"}]}`)
                        bedrockServer.executeCommand(`tellraw "${actor.getName()}" {"rawtext":[{"text":"§l§d[ §fServer §d] §r: 해당 유저의 경고가 음수이기때문에 0으로 바꿨습니다! §c해당 유저의 현재 경고 수: §f( ${read2.warning}/${warningban} )"}]}`)
                    } else {
